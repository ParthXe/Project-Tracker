...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $task_list = DB::select('select projects.id as projectId, projects.project_name, projects.project_status, task_list.id as taskList_id, task_list.task_name, task_list.task_description, task_list.assigned_user_id, task_list.created_at, users.id as userID, users.name FROM projects INNER JOIN task_list ON projects.id=task_list.project_id INNER JOIN users ON task_list.assigned_user_id=users.id');
